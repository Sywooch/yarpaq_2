<footer>
    <div class="hide-footer">
        <i class="fa fa-angle-down"> </i>
    </div>
    <div class="top-footer">
        <div class="container no-padding-xs">
            <div class="row">

                <div class="col-md-12  no-padding">

                    <div class="col-lg-9 col-md-9 must-hide">
                        <div class=" clearfix">
                            <div class="col-md-3 col-xs-6 col-sm-6 v-margin-25">
                                <h4 class="megabox-group-title">MƏLUMAT</h4>
                                <hr>
                                <ul class="links">
                                    <li class="">
                                        <a href="#">
                                            Satıcılar və Alıcılar üçün
                                        </a>


                                    </li>
                                    <li>
                                        <a href="#">
                                            Bizim haqqımızda
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Ödəniş və Çatdırılma
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Məxfilik Siyasəti
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Qaydalar və Şərtlər
                                        </a>

                                    </li>
                                </ul>
                            </div>
                            <div class="col-md-3 col-xs-6 col-sm-6  v-margin-25">
                                <h4 class="megabox-group-title">MƏLUMAT</h4>
                                <hr>
                                <ul class="links">
                                    <li class="">
                                        <a href="#">
                                            Satıcılar və Alıcılar üçün
                                        </a>


                                    </li>
                                    <li>
                                        <a href="#">
                                            Bizim haqqımızda
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Ödəniş və Çatdırılma
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Məxfilik Siyasəti
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Qaydalar və Şərtlər
                                        </a>

                                    </li>
                                </ul>
                            </div>
                            <div class="col-md-3 col-xs-6 col-sm-6  v-margin-25 account-footer">
                                <h4 class="megabox-group-title">MƏLUMAT</h4>
                                <hr>
                                <ul class="links">
                                    <li class="">
                                        <a href="#">
                                            Satıcılar və Alıcılar üçün
                                        </a>


                                    </li>
                                    <li>
                                        <a href="#">
                                            Bizim haqqımızda
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Ödəniş və Çatdırılma
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Məxfilik Siyasəti
                                        </a>

                                    </li>
                                    <li>
                                        <a href="#">
                                            Qaydalar və Şərtlər
                                        </a>

                                    </li>
                                </ul>
                            </div>
                            <div class="col-md-3 col-xs-6 col-sm-6  v-margin-25 contacts-footer">
                                <h4 class="megabox-group-title">BİZİMLƏ ƏLAQƏ</h4>
                                <hr>
                                <ul class="links contact-info">
                                    <li class="">
                                        <i class="fa fa-mobile " aria-hidden="true"></i>+994 12 310 30 03
                                    </li>

                                    <li>
                                        <i class="icons-envelope"></i>support@yarpaq.az

                                    </li>
                                    <li>
                                        <i class="icons-whatsapp"></i> +994 55 555 55 55

                                    </li>
                                    <li class="hidden-xs">
                                            <span>
                                                Yarpaq.az © 2016-2017
                                            </span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3  v-margin-25 footer-soc">
                        <h4 class="megabox-group-title">BİZİ İZLƏYİN</h4>
                        <hr>
                        <ul class="links contact-info">
                            <li class="col-xs-6 col-sm-6  col-md-12 no-padding">
                                <ul class="socIcons clearfix">
                                    <li><i class="fa fa-facebook-square" aria-hidden="true"></i></li>
                                    <li><i class="fa fa-instagram" aria-hidden="true"></i></li>
                                    <li><i class="fa fa-twitter-square" aria-hidden="true"></i></li>
                                </ul>
                            </li>

                            <li class="col-xs-6 col-sm-6  col-md-12 no-padding xs-margin-negative-20 f-logo">
                                <a href="#" title="yarpaq.az" class="logo">
                                    <img src="/img/logo.png" alt="yarpaq.az" width="106px">
                                </a>

                                <div class="hidden-lg hidden-md "> Yarpaq.az © 2016-2017</div>
                            </li>

                        </ul>
                        <div class="clearfix"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="bottom-footer clearfix">
        <div class="container">
            <div class="row">
                <ul class="list-inline payment-methods  text-center">

                    <li><a href="#"><img src="/img/payment1.png"></a></li>
                    <li><a href="#"><img src="/img/payment2.png"></a></li>
                    <li><a href="#"><img src="/img/payment3.png"></a></li>
                    <li><a href="#"><img src="/img/payment4.png"></a></li>
                    <li><a href="#"><img src="/img/payment5.png"></a></li>
                    <li><a href="#"><img src="/img/payment6.png"></a></li>
                    <li><a href="#"><img src="/img/payment7.png"></a></li>
                </ul>
            </div>
        </div>
    </div>
</footer>